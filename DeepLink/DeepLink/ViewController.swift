...
//  DeepLink
//
//  Created by MuhammadQodir on 16.11.2022.
//
import StoreKit
import UIKit
    ...
    private func createFooter() -> UIView {
        ...
        for  x in 0..<3 {
            let button = UIButton(frame:
                                    CGRect(x: CGFloat(x) * size + (CGFloat(x)+1 * 10),
                                           y: 0,
                                           width: size,
                                           height: size))
            footer.addSubview(button)
            button.tag = x+1
            button.setBackgroundImage(UIImage(named: "app\(x+1)"),
                                      for: .normal)
            
            button.addTarget(self,
                             action: #selector(didTabButton(_ : )),
                             for: .touchUpInside)
